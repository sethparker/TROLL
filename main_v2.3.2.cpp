...
int max_la_scheme;	/* 0 = We apply the max leaf area for each stem individually; 1 = We apply the max leaf area to the sum of all stems in the voxel */
float knock_median;
float knocka;
float shed_median;
float sheda;
...
nblivelianas; /* nb live lianas for each timestep */
...
public:
  ...
  unsigned short l_sp_lab;   /* species label */
  Species *l_s;
  int   l_site;           /* location */
  float l_age; /* age of liana; inaccurate if liana wa initialized from user data */
  unsigned short l_from_Data; /* indicates whether liana was initialized from user data (1) or generated by model (0) */
...
void Tree::Birth(Species *S, int nume, int site0) {
    ...
    /* Identify which pixels are occupied by the tree */
...
void Liana::Birth(Species *S, int nume, int site0){
  l_site = site0;
  l_NPPneg = 0;
  l_sp_lab = nume;            /* t_sp_lab is the species label of a site. Can be defined even if the site is empty (cf. persistence function defined in Chave, Am Nat. 2001) */
  l_s = S+l_sp_lab;
  l_age = 1;
  l_from_Data =0;
  l_stem.clear();
...
void Tree::BirthFromData(Species *S, int nume, int site0, float dbh_measured) {
    ...
    t_leafarea=t_dens*t_TotLayerVox*t_Crown_Depth;
...
void LianaStem::BirthFromData(Tree *T, Species *S, int hsite, float ldbh, int nume,
  ...
  ls_t.t_sp_lab = nume;
  ls_t.t_NPPneg=0;
  ls_t.t_s = S+ls_t.t_sp_lab;
  ls_t.t_age = 1;          //value not correct, but generally not problematic, used mainly as diagnostic variable and as indicator of whether tree is alive or not (death itself is independent of age), BUT: use as diagnostic variable cannot be ensured anymore and be careful if conditioning on t_age (e.g. for maturation)
  ls_t.t_from_Data = 1;    //indicates that tree stems from data (and therefore t_age could not be used, etc.)
  ls_t.t_hurt = 0;
  ls_t.t_ddbh=0.0;
...
void Liana::BirthFromData(Tree *T, Species *S, int nume, int site0, float dbh_measured, int nhost, 
  ...
  // Specification of this liana. First, here is information common to all stems.
    l_sp_lab = nume;
    l_s = S+l_sp_lab;
    l_site = site0;
...
void LianaStem::CalcLAI(){
  ...
  cout << "layer: " << ls_t.t_leafareaLayer[crown_top-crown_base] << " " << ls_t.t_leafarea << endl;
}
	...
	layer_lai -= LAI3D[h][t_site+SBORD];
      ...
      t_T    = tmax - LookUp_T[intabsorb];
        ...
        for(int col=max(0,col0-radius_int);col<min(cols,col0+radius_int+1);col++) {
		  ...
		  absorb_below = 0;
		  if (h+1 < HEIGHT){
		    absorb = minf(LAI3D[h+1][col+cols*row+SBORD],19.5);
		    lai_sum -= LAI3D[h+1][col+cols*row+SBORD];
		  }
		  if (h < HEIGHT) {
		    absorb_below = minf(LAI3D[h][col+cols*row+SBORD],19.5);
...
void Tree::UpdateLeafDynamics(Tree *ls_host=NULL) {
  ...
  if(!t_s->s_liana){
	...
	for(int row=max(0,center_x-crown_r);row<=min(rows-1,center_x+crown_r);row++) {
	  ...
	  if(diffx*diffx + diffy*diffy <= crown_r*crown_r){
	    if(max_la_scheme == 0){
	      current_lai = t_LAvox[hh-crown_bot][CRMAX+diffy][CRMAX+diffx];
	    }else{
	      if(max_la_scheme == 1){
	...
	for(int row=max(0,center_x-crown_r);row<=min(rows-1,center_x+crown_r);row++) {
	  ...
	  if(diffx*diffx + diffy*diffy <= crown_r*crown_r){
	    ...
	    la_capacity += maxf(0.,vox_la_max-current_lai);
	  }
	}
      }
    }
    la_to_distribute = minf(la_capacity,excess_la);


    if(l_growth_scheme == 0){
    ...
    if(l_growth_scheme == 1){
      // Make the list of indices
      std::vector<int> my_hh;
      std::vector<int> my_y;
      std::vector<int> my_x;
      std::vector<int> my_site;
      for(int hh=crown_top;hh>=crown_bot;hh--){
	for(int col=max(0,center_y-crown_r);col<=min(cols-1,center_y+crown_r);col++) {
	  int diffy = col - center_y;
	  for(int row=max(0,center_x-crown_r);row<=min(rows-1,center_x+crown_r);row++) {
	    ...
	    if(diffx*diffx + diffy*diffy <= crown_r*crown_r){
	      my_hh.push_back(hh);
	      my_y.push_back(CRMAX+diffy);
	      my_x.push_back(CRMAX+diffx);
	      my_site.push_back(col+cols*row+SBORD);
	    }
	  }
	}
      }
      int which_cell;
      while(my_hh.size() > 0){
	which_cell = trunc(genrand2() * my_hh.size());
	if(max_la_scheme == 0){
	  current_lai = t_LAvox[my_hh.at(which_cell)-crown_bot][my_y.at(which_cell)][my_x.at(which_cell)];
	}else{
	  if(max_la_scheme == 1){
	    current_lai = LAI3D[my_hh.at(which_cell)][my_site.at(which_cell)] - 
	...
	t_leafarea += increment;
	excess_la -= increment;
	my_hh.erase(my_hh.begin()+which_cell);
	my_x.erase(my_x.begin()+which_cell);
	my_y.erase(my_y.begin()+which_cell);
	my_site.erase(my_site.begin()+which_cell);
      }
    }
...
void Liana::Update(){
  ...
  if(l_age){
    ...
    if(_NDD)
      death = int(genrand2()+l_s->DeathRateNDD(0, 0,l_NPPneg, 0));
    else
      death = int(genrand2()+l_s->DeathRate(0, 0, l_NPPneg));
...
void InitialiseFromData(){
    ...
    float col_data, row_data, dbh_measured;                            // values needed for tree initialisation
    ...
    while ((In >> col_data >> row_data >> dbh_measured  >> sp_lab_data) && data_read < sites)       // restricting to data sets with a maximum number of values corresponding to no. of sites
    {
      ...
      nhosts = 0;
      if(S[sp_lab_data].s_liana)In >> nhosts;
    ...
    cout << "NBlianas from Data:\t" << nblivelianas << "\n";
...
void AllocMem() {
    ...
    for(spp=1;spp<=numesp;spp++) {
      if(!S[spp].s_liana)HEIGHT = max(HEIGHT,1+int(S[spp].s_hmax*S[spp].s_dmax*1.5/(S[spp].s_dmax*1.5+S[spp].s_ah)));  //   in number of vertical cells
    }
